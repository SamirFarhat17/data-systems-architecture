...
struct zone
{
    ...
    uint32_t size;               // no. of elements in current zone
};
...
class zonemap
{
    ...
    uint32_t num_zones;             // keeps track of the number of zones/partitions to the array/vector of elements
    uint32_t num_elements_per_zone; // keeps track of number of elements per zone (items in every zone has to be less than or equal to this counter)
